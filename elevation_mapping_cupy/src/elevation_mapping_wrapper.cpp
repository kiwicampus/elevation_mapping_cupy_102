...
void ElevationMappingWrapper::setParameters()
{
    ...
    for (size_t i = 0; i < paramNames.size(); i++)
    {
        ...
        else if (type == "bool")
        {
            ...
            if (node_->get_parameter(name, param))
            {
                RCLCPP_INFO(node_->get_logger(), "Retrieved parameter: %s value: %s", name.c_str(),
                            param ? "true" : "false");
                param_.attr("set_value")(name, param);
    ...
    if (node_->get_parameters("pointcloud_channel_fusions", pointcloud_channel_fusions_params))
    {
        ...
        {
            RCLCPP_INFO(node_->get_logger(), "pointcloud_channel_fusions Key: %s, Value: %s",
                        std::string(py::str(item.first)).c_str(), std::string(py::str(item.second)).c_str());
        }
    }
    else
    {
    ...
    if (node_->get_parameters("image_channel_fusions", image_channel_fusions_params))
    {
        ...
        {
            RCLCPP_INFO(node_->get_logger(), "image_channel_fusions Key: %s, Value: %s",
                        std::string(py::str(item.first)).c_str(), std::string(py::str(item.second)).c_str());
        }
    }
    else
...
void ElevationMappingWrapper::input(const RowMatrixXd& points, const std::vector<std::string>& channels,
                                    const RowMatrixXd& R, const Eigen::VectorXd& t, const double positionNoise,
...
void ElevationMappingWrapper::input_image(const std::vector<ColMatrixXf>& multichannel_image,
                                          const std::vector<std::string>& channels, const RowMatrixXd& R,
                                          const Eigen::VectorXd& t, const RowMatrixXd& cameraMatrix,
                                          const Eigen::VectorXd& D, const std::string distortion_model, int height,
                                          int width)
{
    py::gil_scoped_acquire acquire;
    map_.attr("input_image")(multichannel_image, channels, Eigen::Ref<const RowMatrixXd>(R),
                             Eigen::Ref<const Eigen::VectorXd>(t), Eigen::Ref<const RowMatrixXd>(cameraMatrix),
                             Eigen::Ref<const Eigen::VectorXd>(D), distortion_model, height, width);
}
...
void ElevationMappingWrapper::get_grid_map(grid_map::GridMap& gridMap,
                                           const std::vector<std::string>& requestLayerNames)
{
    ...
    if (enable_normal_color_)
    {
        ...
        map_.attr("get_normal_ref")(Eigen::Ref<RowMatrixXf>(normal_x), Eigen::Ref<RowMatrixXf>(normal_y),
                                                         ...
                                                         std::vector<Eigen::Vector2d>& untraversable_polygon)
{
    ...
    const int untraversable_polygon_num =
        map_.attr("get_polygon_traversability")(Eigen::Ref<const RowMatrixXf>(polygon_m),
