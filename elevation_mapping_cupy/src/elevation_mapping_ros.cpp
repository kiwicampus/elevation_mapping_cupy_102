    ...
    , isGridmapUpdated_(false)
{
    ...
    RCLCPP_INFO(this->get_logger(), "initialize_frame_id: %s",
                initialize_frame_id_.empty() ? "[]" : initialize_frame_id_[0].c_str());
    RCLCPP_INFO(this->get_logger(), "initialize_tf_offset: [%f, %f, %f, %f]", initialize_tf_offset_[0],
                initialize_tf_offset_[1], initialize_tf_offset_[2], initialize_tf_offset_[3]);
    ...
    RCLCPP_INFO(this->get_logger(), "enable_normal_arrow_publishing: %s",
                enableNormalArrowPublishing_ ? "true" : "false");
    RCLCPP_INFO(this->get_logger(), "enable_drift_corrected_TF_publishing: %s",
                enableDriftCorrectedTFPublishing_ ? "true" : "false");
    RCLCPP_INFO(this->get_logger(), "use_initializer_at_start: %s", useInitializerAtStart_ ? "true" : "false");
    RCLCPP_INFO(this->get_logger(), "always_clear_with_initializer: %s",
                alwaysClearWithInitializer_ ? "true" : "false");
    ...
    if (!this->get_parameters("subscribers", subscriber_params))
    {
        RCLCPP_FATAL(this->get_logger(),
                     "There aren't any subscribers to be configured, the elevation mapping cannot be configured. Exit");
        rclcpp::shutdown();
    }
    if (!this->get_parameters("publishers", publisher_params))
    {
        RCLCPP_FATAL(this->get_logger(),
                     "There aren't any publishers to be configured, the elevation mapping cannot be configured. Exit");
        rclcpp::shutdown();
    }
    ...
    for (const auto& sub_name : unique_sub_names)
    {
        ...
        if (this->get_parameter("subscribers." + sub_name + ".data_type", data_type))
        {
            ...
            if (data_type == "image")
            {
                ...
                //   camera_topic.erase(ind - 1, camera_topic.length());                // We remove the hint from the
                //   topic
                // } else {
                ...
                auto img_sub =
                    this->create_subscription<sensor_msgs::msg::Image>(camera_topic, sensor_qos, img_callback);
                ...
                auto img_info_sub =
                    this->create_subscription<sensor_msgs::msg::CameraInfo>(info_topic, sensor_qos, img_info_callback);
                ...
                if (this->get_parameter("subscribers." + sub_name + ".channel_name", channel_info_topic))
                {
                    ...
                    auto channel_info_sub = this->create_subscription<elevation_map_msgs::msg::ChannelInfo>(
                        channel_info_topic, sensor_qos, img_channel_callback);
            ...
            else if (data_type == "pointcloud")
            {
                ...
                //     }, {});
                // point_cloud_transport::Subscriber sub = pct.subscribe(pointcloud_topic, 100, callback, {},
                // transport_hint.get())
                ...
                //   auto sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(pointcloud_topic, sensor_qos,
                //   callback);
    ...
    for (const auto& pub_name : unique_pub_names)
    {
        ...
        if (fps > updateGridMapFps)
        {
            RCLCPP_WARN(this->get_logger(),
                        "[ElevationMappingCupy] fps for topic %s is larger than map_acquire_fps (%f > %f). The topic "
                        "data will be only updated at %f "
                        "fps.",
    ...
    updater_->setHardwareID("elevation_mapping");
    updater_->add("status", this, &ElevationMappingNode::diagnosticTask);
    if (publishStatisticsFps > 0.0)
    {
        updater_->setPeriod(1.0 / publishStatisticsFps);
    }
    ...
    rawSubmapService_ = this->create_service<grid_map_msgs::srv::GetGridMap>(
        "get_raw_submap",
        std::bind(&ElevationMappingNode::getSubmap, this, std::placeholders::_1, std::placeholders::_2));
    ...
    clearMapWithInitializerService_ = this->create_service<std_srvs::srv::Empty>(
        "clear_map_with_initializer",
        std::bind(&ElevationMappingNode::clearMapWithInitializer, this, std::placeholders::_1, std::placeholders::_2));

    initializeMapService_ = this->create_service<elevation_map_msgs::srv::Initialize>(
        "initialize",
        std::bind(&ElevationMappingNode::initializeMap, this, std::placeholders::_1, std::placeholders::_2));

    setPublishPointService_ = this->create_service<std_srvs::srv::SetBool>(
        "set_publish_points",
        std::bind(&ElevationMappingNode::setPublishPoint, this, std::placeholders::_1, std::placeholders::_2));

    // checkSafetyService_ = this->create_service<std_srvs::srv::Empty>(
    //     "check_safety", std::bind(&ElevationMappingNode::checkSafety, this, std::placeholders::_1,
    //     std::placeholders::_2));
...
void ElevationMappingNode::publishMapOfIndex(int index)
{
    ...
    {  // need continuous lock between adding layers and converting to message. Otherwise updateGridmap can reset the
       // data not in map_layers_all_
...
void ElevationMappingNode::imageChannelCallback(const elevation_map_msgs::msg::ChannelInfo::SharedPtr channel_info,
                                                const std::string& key)
{
                                           ...
                                           const std::vector<std::string>& channels)
{
    ...
    RCLCPP_DEBUG(this->get_logger(), "Voxel grid filtered point cloud from %d points to %d points.",
                 static_cast<int>(raw_pcl_pc->width * raw_pcl_pc->height),
                 static_cast<int>(pcl_pc->width * pcl_pc->height));
                                      ...
                                      const std::vector<std::string>& channels)
{
    ...
    if (total_channels != multichannel_image.size())
    {
        RCLCPP_ERROR(this->get_logger(),
                     "Mismatch in the size of multichannel_image (%d), channels (%d). Please check the input.",
                     multichannel_image.size(), channels.size());
        for (const auto& channel : channels)
        {
...
void ElevationMappingNode::imageCallback(const sensor_msgs::msg::Image::SharedPtr image_msg, const std::string& key)
{
    ...
    RCLCPP_DEBUG(this->get_logger(), "ElevationMap imageChannelCallback processed an image in %f sec.",
...
void ElevationMappingNode::updatePose()
{
    ...
    {
        transformStamped =
            tfBuffer_->lookupTransform(mapFrameId_, baseFrameId_, tf2::TimePointZero, tf2::durationFromSec(1.0));
...
bool ElevationMappingNode::getSubmap(const std::shared_ptr<grid_map_msgs::srv::GetGridMap::Request> request,
                                     std::shared_ptr<grid_map_msgs::srv::GetGridMap::Response> response)
{
    ...
    if (requestedFrameId != mapFrameId_)
    {
        ...
        try
        {
            const auto& timeStamp = this->now();
            transformStamped =
                tfBuffer_->lookupTransform(requestedFrameId, mapFrameId_, timeStamp, tf2::durationFromSec(1.0));
    ...
    RCLCPP_DEBUG(this->get_logger(), "Elevation submap request: Position x=%f, y=%f, Length x=%f, y=%f.",
                 requestedSubmapPosition.x(), requestedSubmapPosition.y(), requestedSubmapLength(0),
                 requestedSubmapLength(1));
...
void ElevationMappingNode::clearMapWithInitializer(const std::shared_ptr<std_srvs::srv::Empty::Request> request,
                                                   std::shared_ptr<std_srvs::srv::Empty::Response> response)
{
    RCLCPP_INFO(this->get_logger(), "Clearing map with initializer");
    map_->clear();
...
void ElevationMappingNode::updateVariance() { map_->update_variance(); }

void ElevationMappingNode::updateTime() { map_->update_time(); }

void ElevationMappingNode::diagnosticTask(diagnostic_updater::DiagnosticStatusWrapper& status)
{
    auto now = this->now();
    ...
    if (dt > 0.0)
    {
        status.add("pointcloud_process_fps", std::to_string(pointCloudProcessCounter_ / dt));
    }
    else
    {
        status.add("pointcloud_process_fps", "0.0");
    }
...

    {
        ...
        for (auto const& [key, val] : processingAccumulatedTime_)
        {
            ...
            if (count > 0)
            {
                status.add(key + "_avg_time_ms", std::to_string((val / count) * 1000.0));
            }
            else
            {
                status.add(key + "_avg_time_ms", "0.0");
            }
    ...
    if (request->type == elevation_map_msgs::srv::Initialize::Request::POINTS)
    {
        ...
        for (const auto& point : request->points)
        {
            ...
            if (mapFrameId_ != pointFrameId)
            {
                ...
                {
                    transformStamped =
                        tfBuffer_->lookupTransform(mapFrameId_, pointFrameId, timeStamp, tf2::durationFromSec(1.0));
                    transformationBaseToMap = tf2::transformToEigen(transformStamped);
                } catch (tf2::TransformException& ex)
                {
                    RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 30000, "%s", ex.what());
...
visualization_msgs::msg::Marker ElevationMappingNode::vectorToArrowMarker(const Eigen::Vector3d& start,
